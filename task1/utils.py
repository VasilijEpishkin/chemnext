"""Вспомогательные функции: загрузка молекул из SDF и настройка логирования."""
import logging
...
def load_sdf_molecules(sdf_file: Path) -> List[Tuple[Chem.Mol, str]]:
    """Загружает молекулы из SDF-файла с сохранением 3D-координат.

    Returns:
        Список кортежей (RDKit Mol, имя_молекулы).
    """
    ...
    for i, mol in enumerate(supplier):
        ...
        if mol.GetNumConformers() == 0:
            logging.warning("Молекула '%s' не имеет 3D-координат, пропуск", name)
            continue
